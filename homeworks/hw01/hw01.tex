...
\date{Due by email 11:59PM September 7, \the\year \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\easysubproblem{Derive the CDF $F_X(x)$ of $X \sim \geometric{p}$ from first principles. CDF's are allowed to be the \qu{old} flavor. }\spc{4}

\easysubproblem{Derive the survival function $S_X(x)$ of $X \sim \geometric{p}$ using the result from the previous question. Survival functions are allowed to be the \qu{old} flavor. }\spc{1}
