...
\intermediatesubproblem{Let $X \sim \geometric{p}$. Assume $\expe{X} = \frac{1-p}{p}$. Find $\var{X}$ from the definition of variance.}\spc{11}
...
\hardsubproblem{For continuous rv $X \sim f_X(x)$, given the Fourier inversion theorem, if $\phi_X(t) \in L^1$ then prove P6. Try to do this yourself first. Hint: translate the Fourier formulas (the ones with $g$ and $\hat{g}$ into our vocabulary and then use $u$-substitutions.}\spc{0}
