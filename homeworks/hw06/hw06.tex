...
\problem{We will practice conditional and marginal densities. Let $X \sim \stduniform$ and $Y\,|\,X = x \sim \uniform{0}{x^2}$.}

\begin{enumerate}

\intermediatesubproblem{Draw an illustration of the JDF $f_{X,Y}(x,y)$.}\spc{4}

\hardsubproblem{Find $f_Y(y)$.}\spc{4}

\hardsubproblem{Draw an illustration of $f_Y(y)$.}\spc{4}

\hardsubproblem{Find $f_{X|Y}(x, y)$.}\spc{4}

\hardsubproblem{Draw an illustration of  $f_{X|Y}(x, 0.6)$.}\spc{4}
\end{enumerate}
...
\inthenotessubproblem{Prove the PMF of $X \sim \poisson{\lambda}$ using the limit as $n \rightarrow \infty$ and let $p = \overn{\lambda}$.}\spc{10}


\hardsubproblem{Let $X_1, X_2 \iid \geometric{p}$ and $T = X_1 + X_2$. Find the PMF of $X_1~|~T = t$. It will be a brand name random variable.}\spc{8}


\intermediatesubproblem{[MA] Let $X_1, X_2 \iid \binomial{n}{p}$ and $T = X_1 + X_2$. Show that $X_1~|~T = t$ is hypergeometric distributed (see \href{https://en.wikipedia.org/wiki/Hypergeometric_distribution}{here}).  Note we did not / will not study the hypergeometric further and it will not be covered on any exams.}\spc{8}
...
\inthenotessubproblem{If $X \sim \betanot{\alpha}{\beta}$ and $Y\,|\,X = x \sim \binomial{n}{x}$ where $n$ is considered known in advance. Find $p_Y(y)$.}\spc{8}
...
proves our result as the lhs is the CDF for $Y \sim \binomial{n}{x}.~\blacksquare$
